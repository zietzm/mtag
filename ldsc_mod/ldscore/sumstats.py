...
def estimate_rg(args, log):
    ...
    for k, p1_orig in enumerate(rg_list_1):
        ...
        out_prefix = (
            args.out + rg_files[k]
        )  # YYY make --out writing optional with dataframes
        if user_frames:
            M_annot, w_ld_cname, ref_ld_cnames, sumstats, _ = _read_ld_sumstats(
                args, log, None, alleles=True, dropna=True, sumstats=p1
            )
        else:
            M_annot, w_ld_cname, ref_ld_cnames, sumstats, _ = _read_ld_sumstats(
        ...
        for i in range(k, n_pheno):
            ...
            log.log(
                "Computing rg for phenotypes {K}/{N}-{I}/{N}".format(
                    K=k + 1, I=i + 1, N=len(rg_paths)
                )
            )
            # try:
            if True:
                if user_frames:
                    loop = _read_other_sumstats(
                        args, log, None, sumstats, ref_ld_cnames, sumstats2=p2
                    )
                else:
    ...
    if args.rg_mat:
        X = np.array(
            [None for a in range(n_pheno) for b in range(n_pheno)], dtype=object
        ).reshape(n_pheno, n_pheno)
