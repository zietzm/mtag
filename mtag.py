...
def load_and_merge_data(args):
    ...
    for p in range(P):
        ...
        STRAND_AMBIGUOUS_SET = [
            x
            for x in list(allele_info.STRAND_AMBIGUOUS.keys())
...
def numerical_omega(args, Zs, N_mats, sigma_LD, omega_start):
    ...
    if args.perfect_gencov:
        return np.sqrt(
            np.outer(np.exp(opt_results.x), np.exp(opt_results.x))
        ), opt_results
    else:
...
def write_summary(args, Zs, Ns, Fs, mtag_betas, mtag_se, mtag_factor):
    ...
    for p in range(P):
        ...
        if args.meta_format:
            comb_df_extract = [
                Ns[y][x] for y in list(Ns.keys()) for x in list(Ns[y].keys()) if x == p
            ]
...
def save_mtag_results_U(args, comb_df):
    ...
    for p in range(args.P):
        ...
        comb_df_extract = [
            comb_df[y][x]
            for y in list(comb_df.keys())
            for x in list(comb_df[y].keys())
            if x == p
        ]
        out_df = pd.concat(comb_df_extract, axis=0)
        M_0 = out_df.shape[0]
...
def compute_fdr(prob, t, omega, sigma, S, Ns, N_counts, p_threshold):
    ...
    for k in range(len(S)):
        ...
        Prob_signif_cond_t[k] = (
            np.sum(2 * scipy.stats.norm.sf(z_threshold, loc=0, scale=sd) * N_counts)
            / float(np.sum(N_counts))
